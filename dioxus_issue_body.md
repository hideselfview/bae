...
## Root Cause
The JavaScript serialization code in `dioxus-interpreter-js` sends `type` (without underscore) in `SerializedDataTransferItem`, but the Rust struct in `dioxus-html` expects `type_` (with underscore) because `type` is a reserved keyword in Rust.

**Location**: `dioxus-html/src/data_transfer.rs` line ~104

**JavaScript code** (sends `type`):
```javascript
items.push({kind:item.kind,type:item.type,data})
```

**Rust code** (expects `type_`):
```rust
pub struct SerializedDataTransferItem {
    pub kind: String,
    pub type_: String,  // <-- Missing #[serde(rename = "type")]
    pub data: String,
}
```
...
**Temporary fix**: Apply the above change to `~/.cargo/registry/src/index.crates.io-*/dioxus-html-0.7.0/src/data_transfer.rs` until this is fixed upstream.
