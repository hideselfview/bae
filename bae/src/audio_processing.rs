...
impl AudioProcessor {
    ...
    pub fn extract_track_from_flac(
        ...
        println!("AudioProcessor: Extracting track from {}ms to {}ms", start_time_ms, end_time_ms);
        ...
        println!("AudioProcessor: Seeking to sample {} ({}ms)", start_sample, start_time_ms);
        
        // Seek to start position
        let seconds = (start_time_ms / 1000) as u8;
        let nanoseconds = ((start_time_ms % 1000) * 1000000) as u32;
        let seek_time = Time::from_ss(seconds, nanoseconds)
            .ok_or_else(|| AudioProcessingError::SeekingFailed("Invalid time format".to_string()))?;
        format_reader.seek(symphonia::core::formats::SeekMode::Accurate, symphonia::core::formats::SeekTo::Time { time: seek_time, track_id: Some(track_id) })?;
    ...
    /// Create a minimal FLAC file from PCM data
    /// This is a simplified version - in production we'd want proper FLAC encoding
    pub fn create_flac_from_pcm(
        pcm_data: &[u8],
        sample_rate: u32,
        channels: u16,
        bits_per_sample: u16,
    ) -> Result<Vec<u8>, AudioProcessingError> {
        // For now, return the PCM data with a minimal header
        // TODO: Implement proper FLAC encoding using symphonia or flac crate
        
        // Create a simple header indicating this is raw PCM
        let mut result = Vec::new();
        
        // Add a simple marker to indicate this is processed audio
        result.extend_from_slice(b"PROC"); // "Processed" marker
        result.extend_from_slice(&sample_rate.to_le_bytes());
        result.extend_from_slice(&channels.to_le_bytes());
        result.extend_from_slice(&bits_per_sample.to_le_bytes());
        result.extend_from_slice(&(pcm_data.len() as u32).to_le_bytes());
        result.extend_from_slice(pcm_data);
        
        Ok(result)
    }
    
    /// Get audio properties from FLAC data
    pub fn get_flac_properties(flac_data: &[u8]) -> Result<AudioProperties, AudioProcessingError> {
        let cursor = Cursor::new(flac_data.to_vec()); // Convert to owned Vec to fix lifetime
        let media_source = MediaSourceStream::new(Box::new(cursor), Default::default());
...
/// Audio properties extracted from FLAC files
#[derive(Debug, Clone)]
...
mod tests {
    ...
    #[test]
    fn test_audio_properties_duration() {
        let props = AudioProperties {
            sample_rate: 44100,
            channels: 2,
            bits_per_sample: 16,
            total_samples: 44100 * 60, // 1 minute
        };
        
        assert_eq!(props.duration_ms(), 60000); // 60 seconds = 60000ms
    }
}
