    ...
    match client.get(&json_url).send().await {
        Ok(response) => {
            if response.status().is_success() {
                if let Ok(json) = response.json::<serde_json::Value>().await {
                    if let Some(images) = json.get("images").and_then(|i| i.as_array()) {
                        ...
                        if let Some(first_image) = images.first() {
                            if let Some(image_url) = first_image.get("image").and_then(|i| i.as_str()) {
                                debug!("Using first available cover art: {}", image_url);
                                return Some(image_url.to_string());
                            }
                            if let Some(thumb_url) = first_image.get("thumb").and_then(|t| t.as_str()) {
                ...
                debug!("No cover art found in Cover Art Archive for release {}", release_id);
            } else {
                debug!("Cover Art Archive returned status {} for release {}", response.status(), release_id);
            }
        }
        Err(e) => {
...
pub async fn fetch_cover_art_from_discogs(
    ...
    match discogs_client.get_release(release_id).await {
        Ok(release) => {
            release.cover_image.or_else(|| release.thumb)
        }
        Err(e) => {
