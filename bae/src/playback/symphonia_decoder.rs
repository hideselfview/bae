use crate::playback::StreamingChunkSource;
use std::io::Cursor;
...
impl TrackDecoder {
    ...
    /// Create a new decoder from a streaming chunk source
    ///
    ...
    pub fn seek(&mut self, position: std::time::Duration) -> Result<(), DecoderError> {
        ...
        // Fallback: when seeking fails (ForwardOnly), return error so service can handle it
        // The service will load chunks at the target position and recreate the decoder
        tracing::warn!(
            "Seek failed (ForwardOnly), returning error for service to handle. Target: {}s",
            position_seconds
        );

        // Return error so PlaybackService knows to load chunks at target position
        Err(DecoderError::SeekFailed(format!(
            "ForwardOnly seek not supported at position {}s",
            position_seconds
        )))
    }
